...
urxvt-full, terminology, sixel, kitty, ueberzug, ueberzugpp, chafa.
See I<PREVIEWS> section.

C<ueberzugpp> is a drop-in C++ replacement for C<ueberzug> with Wayland,
X11 and Kitty support (L<https://github.com/jstkdng/ueberzugpp>).

C<chafa> renders images as Unicode/ANSI art and works in any terminal
including SSH sessions without X11 (L<https://hpjansson.org/chafa/>).
...
=item nerd_font_version [integer]

Version of the Nerd Font installed in your terminal. Used to measure the
display width of icon glyphs correctly.

Possible values:

=over

=item B<0> (default) – no Nerd Font / unknown.

=item B<2> – Nerd Fonts v2.x (icons are 1 cell wide).

=item B<3> – Nerd Fonts v3.x (icons are 2 cells wide; spacing corrected
automatically).

=back

See also C<show_icons>.

=item show_cursor [bool]

Always show the terminal cursor?

=item show_icons [bool]

Show Nerd Font icons next to filenames? Requires a Nerd Font
(L<https://www.nerdfonts.com>) active in your terminal emulator.

When enabled, ranger switches to the C<icons> linemode and (if
C<nerd_font_version> is 2 or 3) replaces plain-Unicode VCS symbols with
Nerd Font glyphs. Can also be activated manually with C<:set linemode icons>.

See also C<nerd_font_version>.
